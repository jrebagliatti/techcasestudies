...
1.	Install the `mobile-center-cli` so the release agent can execute commands against mobile-center: execute npm with arguments `install -g mobile-center-cli`.

2.	Sign in to mobile-center using build definition variables: execute mobile-center with arguments `login -u $(mobilecenteruser) -p $(mobilecenterpassword)`.
