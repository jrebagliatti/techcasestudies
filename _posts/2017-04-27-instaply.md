...
- [Language Understanding Intelligent Service (LUIS)](https://luis.ai), a natural-language parsing service.
...
2. Instaply's webhook sends a payload containing `fromCustomer`, `threadId`, and `query string`.
    ...
    1. If the webhook is from a customer, we continue. Messages from the sales representative are filtered here.
    2. The `threadId` is checked in the repository.
    
        * If new, record in repository and create connection with bot.
        * If exists, check connection with bot and continue.
        
    3. The message is sent from the Direct Line client to the bot.
