...
Let's have a look at one sample Lua script that polls the ABUS Secvest alarm panel every minute for the current status:
...
![IoT Hub custom endpoints]({{ site.baseurl }}/images/2017-03-17-ABUS/IoTHubEndpoints.png)

Then we created a special route that filters out Log messages and sends them to the Log endpoint.

![IoT Hub routes]({{ site.baseurl }}/images/2017-03-17-ABUS/IoTHubRoutes.png)
...
The queries that insert data into the "DeviceState" and "DeviceMessages" tables basically look the same but they still create different results:
...
![Azure Tables]({{ site.baseurl }}/images/2017-03-17-ABUS/StreamAnalyticsDeviceState.png)
...
![Azure Tables]({{ site.baseurl }}/images/2017-03-17-ABUS/StreamAnalyticsDeviceMessages.png)

We then can have a first view of what's in the Azure Tables using Storage Explorer. For the DeviceState table we can see that the current Secvest state is reflected by one row.

![Azure Tables]({{ site.baseurl }}/images/2017-03-17-ABUS/AzureTablesDeviceState.png)

In the DeviceMessages table we see all incoming messages.

![Azure Tables]({{ site.baseurl }}/images/2017-03-17-ABUS/AzureTablesDeviceMessages.png)

### Azure Functions

In section "Azure IoT Hub" we wrote about filtering out the Log messages into a seperate EventHub queue. This EventHub queue is monitored by an Azure Function that processes the log messages. For debugging purposes we log out every message and apply another filtering on "Critial" errors, which might be important to forward to another queue.

The azure function is written in C#. You can see from the screenshot that it processes and outputs log entries into the Azure Function Logs window:

![Azure Tables]({{ site.baseurl }}/images/2017-03-17-ABUS/AzureFunctionsLogHandler.png)

The code itself is pretty straightforward. It takes an EventHub message, creates an object by using Newtonsoft JsonConvert and checks if it is a Log message and then addionally if it is a critical error.
...
Finally, looking at the AppService Web App we can see which devices are connected to the cloud.

![Device Portal]({{ site.baseurl }}/images/2017-03-17-ABUS/screenshot_device_portal.png)

#### Device Messages

We can send messages to the device, which get handled by the AzureIoTHubDevice's MessageReceiverLoop.

![Device Portal]({{ site.baseurl }}/images/2017-03-17-ABUS/WebPortalSendMessage.png)

#### Device Remote Configuration

In the "Edit" dialog for a device, there is an Editor field where one can put a http url, which points to a new configuration file for the device.

![Device Portal]({{ site.baseurl }}/images/2017-03-17-ABUS/WebPortalConfigurationFile.png)

As soon as this information is saved, the device twin gets updated with the new url and on the device side the new configuration gets loaded (by restarting the app):
...
In our engagement with ABUS, we successfully managed to connect the Secvest alarm panel to Azure IoT Hub using Windows 10 IoT Core and a UWP app as the middleware to broker messages and get a secure and reliable messaging channel. ABUS at a very early stage of the project decided to contract for the Microsoft Azure Cloud Germany because of the unique security and legal model as well as the trusted Microsoft Azure technology stack.

It looks like the solution is usable for taking it one step further and integrate more ABUS devices and create a re-usable solution for other clients. From our side we will continue to develop the solution to be re-usable and have the latest sources on github for future engagements.

ABUS is currently evaluating the next stages of the project with PoCs, which cover several other aspects of the scenarion like video streaming and VoD. Another step is to find the right hardware for the gateway device, which will have requirements that exceed the Raspberri Pi 3. It will need to have an onboard TPM chip for extended security and onboard EMMC instead of the SD-CARD slot.

## Additional resources ##

Github repository of the iot_homesecurity base framework:
- https://github.com/kreuzhofer/iot_homesecurity 

Github repository of the ABUS PoC sourcecodes
- https://github.com/kreuzhofer/iot_homesecurity/tree/ABUS

ABUS Security-Center GmbH & Co. KG Homepage
- https://www.abus-sc.de/
