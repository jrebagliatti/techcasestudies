...
The hackfest took place a few weeks after a Value Stream Map session where we were able to identify some pain points and plan for
...
*Image: VSM of current development process done by the team*
...
**Image:** *SettingsApi Epic has its own release, in which all features changed are shown* 
...
is to orchestrate the process of retrieving a *Tenant* object. In the example, this means validating the parameter and doing a central exception handling
...
the configuration part, and still need to design a page for the user to actually set this value. Some work left on this, but 
...
a significant improvement to the %C&A. It is twice the current team speed, and we expect the lead 
